                    ...
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                            ...
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                <td style="max-width: 10rem;" class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                                    <div class="text-base truncate font-semibold text-gray-900 dark:text-white">{{ $data->article->title }}</div>
                                    <div class="text-sm truncate font-normal text-gray-500 dark:text-gray-400">Admasdasdadsasdasdasdn</div>
                                </td>
                                <td style="max-width: 10rem;" class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                                    <div class="text-base truncate font-semibold text-gray-900 dark:text-white">{{ $data->name }}</div>
                                    <div class="text-sm truncate font-normal text-gray-500 dark:text-gray-400">{{ $data->email }}</div>
                                </td>
                                <td style="max-width: 20rem;" class="p-4 overflow-hidden text-base font-normal text-gray-500 xl:max-w-xs dark:text-gray-400">
                                    <h1 class="line-clamp-4">{{ $data->content }}</h1>
                                </td>
                                <td style="max-width: 6rem;" class="p-4 truncate text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ date('M d, Y', strtotime($data->created_at)) }}</td>
                                <td class="p-4 space-x-2 whitespace-nowrap">
                                    <form action="{{ route('comment.destroy', $data->id)  }}" data-confirm="true" onclick="return confirm('Yakin ingin menghapus data?')" method="post">
                                        @csrf
                                        @method("delete")
